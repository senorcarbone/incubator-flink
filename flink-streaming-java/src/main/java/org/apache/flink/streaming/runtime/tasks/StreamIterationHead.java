...
public class StreamIterationHead<IN> extends OneInputStreamTask<IN, IN> {
	...
	public void init() throws Exception {
		...
		ListStateDescriptor<StreamRecord<IN>> streamRecordListStateDescriptor =
			new ListStateDescriptor<>("upstream-log", getConfiguration().<StreamRecord<IN>>getTypeSerializerOut(getUserCodeClassLoader()));
		getConfiguration().setStreamOperator(new UpstreamLogger(streamRecordListStateDescriptor));
				...
				synchronized (lock) {
					...
					if (nextRecord != null) {
						...
						if (nextRecord.isLeft()) {
							...
							checkpointState(new CheckpointMetaData(nextRecord.right().getId(), nextRecord.right().getTimestamp()));
							flushAndClearLog();
							onSnapshot = false;
						}
	...
	public boolean triggerCheckpoint(CheckpointMetaData checkpointMetaData) throws Exception {
		...
		synchronized (getCheckpointLock()){
			operatorChain.broadcastCheckpointBarrier(checkpointMetaData.getCheckpointId(), checkpointMetaData.getTimestamp());

			if (onSnapshot || !hasFlushed) {
				LOG.debug("Iteration head {} aborting checkpoint {}", getName(), checkpointMetaData.getCheckpointId());
				return false;
			}
			onSnapshot = true;
			return true;
	...
	public static class UpstreamLogger<IN> extends AbstractStreamOperator<IN> implements OneInputStreamOperator<IN, IN> {
		...
		private final ListStateDescriptor<StreamRecord<IN>> stateDescriptor;


		private UpstreamLogger(ListStateDescriptor<StreamRecord<IN>> stateDescriptor) {
		...
		public void open() throws Exception {
			super.open();
			this.listState = getOperatorStateBackend().getOperatorState(stateDescriptor);
		}
