...
import org.apache.flink.streaming.api.datastream.{DataStream => JavaStream, ConnectedDataStream, SingleOutputStreamOperator, GroupedDataStream}
...
class DataStream[T](javaStream: JavaStream[T]) {
   ...
   */
  def connect[T2](dataStream: DataStream[T2]): ConnectedDataStream[T, T2] = javaStream.connect(dataStream.getJavaStream)
