...
import org.apache.flink.streaming.util.nextGenExtractor.ConcatinatedExtract;
import org.apache.flink.streaming.util.nextGenExtractor.FieldsFromArray;
...
public class Delta<DATA> implements NextGenWindowHelper<DATA> {
	...
	public static <DATA> Delta<DATA> of(NextGenDeltaFunction<DATA> deltaFunction, DATA initVal, double threshold) {
		return new Delta<DATA>(deltaFunction, initVal, threshold);
	}

	/**
	 * This method allows to set up the delta function in a readable way.
	 ...
	 * </code>
	 * 
	 * @param threshold the threshold for the delta function
	 * @return instance of a helper class which provides further methods to make all settings in a readable way.
	 */
	...
	public static class DeltaHelper2 {
		...
		public DeltaHelper3 fromTuple(){
			if (laterExtractor==null){
				return new DeltaHelper3(threshold, new FieldsFromTuple(fields));
			} else {
				return new DeltaHelper3(threshold, new ConcatinatedExtract(new FieldsFromTuple(fields), laterExtractor));
			}
		...
		public DeltaHelper3 fromArray(Class clazz){
			if (laterExtractor==null){
				...
				return new DeltaHelper3(threshold, new ConcatinatedExtract(new FieldsFromArray(clazz, fields), laterExtractor));
			}
		}
	}
