...
public class WindowingInvokableTest {
	...
	public void testWindowingInvokableWithTimePolicy() {
		...
		expected.add(12);
		//TODO: Implement a solution which leads to the below result
		//expected.add(12);
		//expected.add(5);
		...
		//Trigger every 2 time units but delay the first trigger by 2 (First trigger after 4, then every 2)
		...
		WindowingInvokable<Integer> invokable = new WindowingInvokable<Integer>(myReduceFunction,triggers,evictions);
		
		ArrayList<Integer> result = new ArrayList<Integer>();
		for (Tuple2<Integer, String[]> t:MockInvokable.createAndExecute(invokable, inputs)){
			result.add(t.getField(0));
		}
	...
	public void testWindowingInvokableWithCountPolicy() {
		...
		LinkedList<EvictionPolicy<Integer>> evictions= new LinkedList<EvictionPolicy<Integer>>();
		//On every 2nd element, remove the oldest 2 elements, but the first time after the 3rd element
		...
		inputs2.add(1);
		inputs2.add(2);
		inputs2.add(-5); //changed this value to make sure it is excluded from the result
		inputs2.add(-3);
		inputs2.add(-4);
	...
	public void testWindowingInvokableWithMultiplePolicies() {
		...
		LinkedList<EvictionPolicy<Integer>> evictions= new LinkedList<EvictionPolicy<Integer>>();
		//On every 2nd element, remove the oldest 2 elements, but the first time after the 3rd element
		evictions.add(new CountEvictionPolicy<Integer>(2));
		evictions.add(new CountEvictionPolicy<Integer>(3));
